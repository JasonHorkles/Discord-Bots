        ...
        new Thread(() -> {
            try (ScheduledExecutorService executor = Executors.newSingleThreadScheduledExecutor()) {
                schedules.add(executor.schedule(() -> {
                    try {
                        Quorum.jda.getGuildById(853775450680590387L).getMembersByEffectiveName(name, true)
                            .get(0).getUser().openPrivateChannel().flatMap(channel -> channel.sendMessage(
                        ...
                        Quorum.jda.getTextChannelById(869009573774761984L).sendMessage(
                                "Messaged **" + name + "** to prepare his lesson for next Sunday: " + title)
                            .queue();
